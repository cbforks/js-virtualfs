...
class Directory extends INode {
  ...
  getName (index) {
    return this._dirIndex.get(index);
  }
   ...
   */
  getEntry (name) {
    return this._iNodeMgr.getINode(this._dir.get(name));
  ...
  renameEntry (oldName, newName) {
    ...
    this._metadata.mtime = now;
    this._metadata.ctime = now;
    let index = this._dir.get(oldName);
  ...
  destructor () {
    ...
    if (this._dir.get('.') !== this._dir.get('..')) {
      this._iNodeMgr.unlinkINode(this._dir.get('..'));
    }
    return;
  }
...
class INodeManager {
   ...
   */
  linkINode (index) {
    ++(this._inodes.get(index).getMetadata().nlink);
    return;
  }
   ...
   */
  unlinkINode (index) {
    let inode = this._inodes.get(index);
    let metadata = inode.getMetadata();
    --metadata.nlink;
    if (metadata.nlink === 0) {
