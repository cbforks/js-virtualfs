...
class VirtualFS {
  ...
  appendFileSync (pathSOrFdIndex, data='undefined', optionsOrEncoding) {
    ...
    if (typeof pathSOrFdIndex === 'number') {
      const fd = this._fdMgr.getFd(pathSOrFdIndex);
      const appendPosition = fd.getINode().data.length;
      this.writeSync(pathSOrFdIndex, data, 0, data.length, appendPosition);
      return;
    }
    ...
    if (navigated.target instanceof File) {
      try {
        navigated.target.write(Buffer.concat(navigated.target.data, data));
      } catch (e) {
        if (e instanceof RangeError) {
          throw new VirtualFSError(errno.code.ENOSPC, pathSOrFdIndex);
        }
        throw e;
      }
    } else {
  ...
  openSync (pathS, flags, mode) {
    ...
    if (!target) {
      ...
      {
        // we need to reset the position of the file descriptor?
        target.write(Buffer.alloc(0));
      }
    }
    // but if you resize the file, where is the file descriptor set?
  ...
  ftruncateSync (fdIndex, len=0) {
    ...
    const iNodeBuffer = iNode.data;
    let newBuffer;
    if (iNodeBuffer.length > len) {
      newBuffer = Buffer.allocUnsafe(len);
      iNodeBuffer.copy(newBuffer, 0, 0, len);
      iNode.write(newBuffer);
    } else if (iNodeBuffer.length < len) {
      newBuffer = Buffer.alloc(len);
      iNodeBuffer.copy(newBuffer, 0, 0, iNodeBuffer.length);
      iNode.write(newBuffer);
    } else {
      // change the mtime and ctime
