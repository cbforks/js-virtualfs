...
class FileDescriptor {
  ...
  // it turns out that write file is a simple matter of opening FD and performing the truncation or whatever, and then writing to it all in one go

  // file descriptors also need to expose truncate operation
  // the fd position should be changed, although this needs testing as well (changed to the end right?) the fd position normally doesn't change unless the overall size is less
  truncate (len=0) {
    const iNode = this._inode;
    switch (true) {
    case iNode instanceof File:
    ...
    metadata.ctime = now;
    metadata.size = data.length;
    this._pos = Math.min(data.length, this._pos);
    return;
  }
